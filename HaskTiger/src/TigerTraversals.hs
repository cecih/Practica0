...
instance Traversable Exp Dec where
  traversable f (FunctionDec xs)           = 
    FunctionDec <$> traverse (\(sym, fs, mbs, e, p)
                                  -> (,,,,) 
                                     <$> pure sym 
                                     <*> pure fs 
                                     <*> pure mbs 
...
instance Traversable Var Exp where
  traversable f = traversable cas
    where cas (VarExp v p)         = VarExp <$> f v <*> pure p
...
countPrints = getSum . travFold caso
  where caso (CallExp s _ _) = if unpack s == "print" then Sum 1
                                 else Sum 0
