...
reorder []                  = return (nop, [])
reorder (e@(Call _ _) : es) = 
  do t <- newTemp
     reorder (Eseq (Move (Temp t) e) (Temp t) : es)
reorder (e : rest) = 
  do (sts, e')  <- doExp e
     (sts', el) <- reorder rest
     if conmute sts' e' then return (sts % sts', e':el)
       else do t <- newTemp
...
traceSchedule (blocks, done) = 
  do mapM_ enterBlock blocks
     ls <- getnext blocks
...
canon tseed lseed frs = 
  let fsTank = firstTank {lgen = lseed, tgen = tseed}
      (res,est) = runState (
                    mapM (\(st,fr) -> 
                              do ss <- canonM st
